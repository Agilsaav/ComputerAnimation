...
GLuint Model::loadTexture(const std::string& strFilename)
{
  ...
  else if (_stricmp(strrchr(strFilename.c_str(),'.'),".tga")==0)
  {
...
bool Model::onInit(const std::string& strFilename)
{
  ...
  std::cout << "iniiii!!" << std::endl;

  //Init shader
  Shader vShader, fShader;

  string vpath = "Res/shaders/basic.vert";
  string fpath = "Res/shaders/basic.frag";
...
void Model::initModelRendering()
{
	...
	for (meshId = 0; meshId < meshCount; meshId++)
	{
		...
		for (submeshId = 0; submeshId < submeshCount; submeshId++)
		{
			...
			{
				TriangleMesh modelMeshTemp;

				std::vector<glm::vec3> vertices;
				std::vector<glm::vec3> normals;
				std::vector<int> faces;
				std::vector<glm::vec2> textCorrd;
				...
				{
					vertices.push_back(glm::vec3(meshVertices[i][0], meshVertices[i][1], meshVertices[i][2]));
					normals.push_back(glm::vec3(meshNormals[i][0], meshNormals[i][1], meshNormals[i][2]));
					textCorrd.push_back(glm::vec2(meshTextureCoordinates[i][0], meshTextureCoordinates[i][1]));

					modelMeshTemp.addVertex(vertices[i]);
					modelMeshTemp.addNormal(normals[i]);
				}
				...
				{
					modelMeshTemp.addTriangle(meshFaces[3 * i][0], meshFaces[3 * i][1], meshFaces[3 * i][2]);
				}

				modelMeshTemp.sendToOpenGLN(basicProgram);

				modelMesh.push_back(modelMeshTemp);
			}
		}
    ...
    for(submeshId = 0; submeshId < submeshCount; submeshId++)
    {
      ...
      if(pCalRenderer->selectMeshSubmesh(meshId, submeshId))
      {
		...
		for (unsigned int i = 0; i < vertexCount; i++)
		{
			...
			modelMesh[count].changeVertex(vertices[i],i);
...
//// DEBUG END
  ...
  //renderMesh(false, true);
  //COMENTAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAT
