...
//	Particle(glm::vec3 pos, glm::vec3 vel, float bouncing = 1.0f, bool fixed = false, int lifetime = -1, glm::vec3 force = glm::vec3(0, 0, 0));
	...
	void setMass(const float& partMass);
	void setDensity(const float& den);
	void setPreasure(const float& pre);
	...
	void computeParticleMass(const float& fluidDen, const float& Vol, const int& numPart);
	void computeParticleDensity(const float& h, const float& fluidDen);
	void computePreasure(const float& fluidDen, const float& Vol, const float& k);
	void computeNN(std::vector<Particle>& particles, const float& dist, unsigned int index);
